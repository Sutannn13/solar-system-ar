...
- Solar overview orientation: apply a native `0 0 90` roll to `solar_system_animation.glb`, then fit and center it, then apply a dedicated `solarPresentation` wrapper with `-42deg` pitch and `-8deg` yaw so the orbit path reads as a horizontal solar-system ellipse on portrait phones instead of a vertical stack or a flat line; interaction may yaw around that wrapper but must not add extra pitch/tilt beyond the base presentation angle.
...
Current baseline: base scales, native GLB roll, wrapper presentation angle, AR.js high-clarity camera profile, and marker smoothing live in `src/ar/scene.ts` (`SOLAR_SYSTEM_MODEL_SCALE`, `SOLAR_FALLBACK_SCALE`, `SOLAR_SYSTEM_NATIVE_ROTATION`, `SOLAR_PRESENTATION_TILT_DEG`, `SOLAR_PRESENTATION_YAW_DEG`, `AR_CAMERA_SOURCE_WIDTH`, `AR_CAMERA_SOURCE_HEIGHT`) with desktop-sized marker fit preserved and smartphone fit capped in `src/main.ts` (`getSolarScaleMultiplier`, `getSolarOverviewTargetSize`) so the solar system does not overwhelm the Hiro marker on portrait phones. The loaded `solar_system_animation.glb` is tidied at runtime by hiding decorative asteroid/dwarf clutter, shrinking the Sun node, centering the fitted model over the Hiro marker, and then presenting the wrapper at a readable orbit angle instead of a fully flat or vertical plane. Detail panel preview uses the selected planet's `modelPath` and `previewScale` from `src/data/planets.ts`; no CSS-drawn planet fallback should sit behind the GLB preview.
